 ...
 * Gemini prompt context. Uses S2A filtering and density compression
 * to pack more signal into fewer tokens.
 * 
 * Based on: QuickSave v9.1 by Kevin Tan (ktg.one)
 * 
 * Integration points:
 *   1. compressEventsForPrompt() → used in gemini.ts before every Gemini call
...
export interface CompressedContext {
  events: string;           // Dense text block for Gemini prompt
...
// ============ EVENT COMPRESSION ============
// Compress event list into a dense text block for Gemini prompts.
...
export function compressEventsForPrompt(
  ...
  // Add edge summary if any found (helps Gemini understand relationships)
  let edgeSuffix = '';
  if (edges.length > 0) {
...
// Recent turns stay raw (Gemini needs exact wording), older turns get
